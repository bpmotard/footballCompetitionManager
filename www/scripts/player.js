...
function formUpdatePlayer() {
    ...
    /**
     * função para ir buscar a posicao/pais do jogar e ficar pre-definido para quando se faz editar jogador
     */
    function setOption(playerAttribute, selectId) {
        var options = document.getElementById(selectId).options;
        for (var i = 0; i < options.length; i++) {
            if (options[i].value === playerAttribute) {
                document.getElementById(selectId).selectedIndex = i;
                break;
            }
            selectedIndex = -1; /*de maneira a não deixar a idade do jogador anterior seleccionada */
        }
    };
    ...
    for (i = checkbox.length - 1; i > -1; i--) {
        if (checkbox[i].checked === true) {
            ...
            setOption(allPlayerList[i].country, "inputUpdatePlayerCountry");
            document.getElementById("inputUpdatePlayerHeight").defaultValue = allPlayerList[i].height;
            setOption(allPlayerList[i].position, "inputUpdatePlayerPosition");
        }
    }
};
...
function formUpdatePlayerAction() {
    ...
    for (i = checkbox.length - 1; i > -1; i--) {
        if (checkbox[i].checked === true) {
            var ageString = pAge;
            var ageFormated = ageString.replace(/-/g, ',');

            allPlayerList[i].birthDate = new Date(ageFormated);
            allPlayerList[i].name = pName;
            allPlayerList[i].age = calcAge(allPlayerList[i].birthDate);   /* necessidade de recalcular a idade*/
